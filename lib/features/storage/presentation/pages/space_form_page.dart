...
class SpaceFormPage extends StatelessWidget {
  const SpaceFormPage({super.key});
  @override
  Widget build(BuildContext context) => BlocProvider(
    ...
    child: AppScaffold(
      ...
      body: BlocListener<StorageFormCubit, StorageFormState>(
        ...
        listener: (context, state) => context.go('/spaces'),
        child: Builder(
          builder: (context) => StorageNodeForm(
            submitLabel: 'Save Space',
            onSubmit: context.read<StorageFormCubit>().createSpace,
          ),
        ),
      ),
    ),
